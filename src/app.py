...
class FileListExtractor(ctk.CTk):
    ...
    def _build_ui(self):
        ...
        ctk.CTkLabel(
            title_frame, text="File List Extractor",
        ...
        self.folder_entry = ctk.CTkEntry(
            row1, placeholder_text="Select a folder...",
        ...
        ctk.CTkCheckBox(row2, text="Include extension in filename",
            variable=self.include_ext_var, font=ctk.CTkFont(size=12),
            text_color="#a0a0a0", fg_color="#1f6feb", hover_color="#388bfd",
            border_color="#444444", checkmark_color="white").pack(side="left", padx=(0, 20))
        self.include_subdir_var = ctk.BooleanVar(value=True)
        ctk.CTkCheckBox(row2, text="Include subdirectories",
            variable=self.include_subdir_var, font=ctk.CTkFont(size=12),
            text_color="#a0a0a0", fg_color="#1f6feb", hover_color="#388bfd",
            border_color="#444444", checkmark_color="white").pack(side="left", padx=(0, 20))
        self.show_full_path_var = ctk.BooleanVar(value=False)
        ctk.CTkCheckBox(row2, text="Show full path",
            variable=self.show_full_path_var, font=ctk.CTkFont(size=12),
            text_color="#a0a0a0", fg_color="#1f6feb", hover_color="#388bfd",
            border_color="#444444", checkmark_color="white").pack(side="left")
        ...
        self.textbox = ctk.CTkTextbox(text_frame,
            fg_color="#111111", text_color="#d0d0d0",
        ...
        btn_inner.pack(side="right", padx=16, pady=10)
        ctk.CTkButton(btn_inner, text="Copy to Clipboard", width=160, height=32,
        ...
        ctk.CTkButton(btn_inner, text="Save as TXT", width=140, height=32,
            fg_color="#2d2d2d", hover_color="#3d3d3d", font=ctk.CTkFont(size=12),
            corner_radius=8, border_width=1, border_color="#444444",
            command=self._save).pack(side="left", padx=(0, 8))
        ctk.CTkButton(btn_inner, text="Clear", width=90, height=32,
    ...
    def _do_extract(self, folder):
        ...
        try:
                ...
                for root, dirs, files in os.walk(folder):
                    ...
                    for f in sorted(files):
                        ...
                        if full_path:
                            ...
                            lines.append(name if rel_root == "." else os.path.join(rel_root, name))
            else:
            ...
            self.status_label.configure(
                text=f"  {total} file(s) found in '{os.path.basename(folder)}'",
                text_color="#3fb950")
    ...
    def _copy(self):
        ...
        if not content:
            messagebox.showwarning("Nothing to copy", "Extract files first.")
            return
    ...
    def _save(self):
        content = self.textbox.get("1.0", "end").strip()
        if not content:
            messagebox.showwarning("Nothing to save", "Extract files first.")
            return
        path = filedialog.asksaveasfilename(
            defaultextension=".txt",
            filetypes=[("Text files", "*.txt"), ("All files", "*.*")],
            initialfile="filelist.txt")
        if path:
